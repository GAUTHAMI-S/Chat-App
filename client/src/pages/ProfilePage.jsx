...
const ProfilePage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    reader.onload = async () => {
      ...
      await updateProfile({ profilePic: base64Image, ...formData })
      navigate('/')
    }
  }
    ...
    <div className='min-h-screen bg-cover bg-no-repeat flex items-center justify-center'>
      ...
      flex items-center justify-between max-sm:flex-col-reverse rounded-lg'>
        <form onSubmit={handleSubmit} className='flex flex-col gap-5 p-10 flex-1'>
          ...
          <input onChange={(e) => setFormData(prev => { return { ...prev, fullName: e.target.value } })}
            type="text" required placeholder='Your name' value={formData.fullName}
            className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-violet-500'
          />
          <textarea
            value={formData.bio}
            onChange={(e) => setFormData(prev => { return { ...prev, bio: e.target.value } })}
            placeholder='Write profile bio' required rows={4}
            className='p-2 border 
          border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-violet-500'></textarea>


          <button type='submit' className='bg-gradient-to-r from-purple-400 to-violet-600 text-white p-2 rounded-full text-lg cursor-pointer'>Save</button>  </form>
